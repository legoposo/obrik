        ...
        <div class="overflow-hidden rounded-2xl bg-white shadow dark:bg-zinc-900">
            <table class="min-w-full divide-y divide-zinc-200 dark:divide-zinc-800">
                ...
                <tbody class="divide-y divide-zinc-200 dark:divide-zinc-800">
                    @forelse($works as $work)
                        @php
                            $statusMap = [
                                ...
                                'finished' => ['label' => 'Concluída', 'class' => 'bg-green-100 text-green-700'],
                                'canceled' => ['label' => 'Cancelada', 'class' => 'bg-red-100 text-red-700'],
                            ];

                            $status = $statusMap[$work->status] ?? [
                                'label' => $work->status,
                        ...
                        <tr>
                            ...
                            <td class="px-6 py-4 text-sm text-zinc-700 dark:text-zinc-200">
                                {{ $work->start_date ? \Carbon\Carbon::parse($work->start_date)->format('d/m/Y') : '-' }}
                            </td>

                            <td class="px-6 py-4 text-sm text-zinc-700 dark:text-zinc-200">
                                {{ $work->expected_end_date ? \Carbon\Carbon::parse($work->expected_end_date)->format('d/m/Y') : '-' }}
                            </td>
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    <form action="{{ route('works.destroy', $work->id) }}"
                                          method="POST"
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-4 text-sm text-zinc-700 dark:text-zinc-200 text-center">
